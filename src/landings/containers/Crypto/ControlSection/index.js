...
import Card from 'landings/common/src/components/Card';
...
import ControlImage from 'landings/common/src/assets/image/crypto/control.jpg';

const Completionist = () => <span className="readMore">You are good to go!</span>;
...
const ControlSection = ({ row, col, title, description, btnStyle, sectionSubTitle, cardArea, readMoreTitle }) => {
  return (
    <ControlWrapper id="control">
      <Container>
        <Box className="row" {...row}>
          <Box className="colleft" {...col} style={{ flexDirection: 'column' }}>
            <Image src={ControlImage} className="controlImage" alt="Control Section Image" />
          </Box>
          <Box className="colright" {...col} {...cardArea}>
            <Text {...sectionSubTitle} />
            <FeatureBlock title={<Heading {...title} />} description={<Text {...description} />} />
            ...
            <Fade up>
              <Box className="countDownSection">
                <Countdown date={Date.now() + 909999999} renderer={renderer} completed={false} />
              </Box>
            </Fade>
            <Box className="countDownButton">
              <Button title="BUY TOKENS" className="countDownMainButton" {...btnStyle} />
              <Button title="35% Bonus" className="countDownDiscountButton" {...btnStyle} />
            </Box>
          </Box>
        </Box>
      </Container>
