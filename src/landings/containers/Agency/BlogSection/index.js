...
const BlogSection = ({ row, sectionHeader, sectionTitle, sectionSubTitle, blogTitle, blogMeta }) => {
  return (
    <BlogSectionWrapper id="blogSection">
      <Container>
        <Box {...sectionHeader}>
          <Text content="PORTFOLIO" {...sectionSubTitle} />
          <Heading content="Meet our work experience from customers" {...sectionTitle} />
        </Box>
        <Box className="row" {...row}>
          {data.blog.map((post, index) => (
            <FeatureBlock
              ...
              icon={<Image src={post.thumbnail_url} alt={`Blog Image ${post.id}`} className="blog__image" />}
              title={
