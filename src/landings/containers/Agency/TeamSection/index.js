    ...
    <TeamSectionWrapper id="teamSection">
      <Container>
        <Box {...sectionHeader}>
          ...
          <Heading content="Meet with team member behind scense" {...sectionTitle} />
        </Box>
          ...
          {data.teamMember.map((member, index) => (
            <Box className="col" {...col} key={`team_key-${index}`}>
              <FeatureBlock
                ...
                icon={
                  <Image src={member.thumbnail_url} alt={`Team member photo ${member.id}`} className="member__photo" />
                }
