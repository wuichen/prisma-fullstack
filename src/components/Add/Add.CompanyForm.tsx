...
import { useCreateOneCompanyMutation } from 'generated';
...
function ContactForm({ baseDelay }: { baseDelay: number }) {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      ...
      const newCompany = await createOneCompanyMutation({
        variables: {
          data: {
            name: company,
            description: details,
            slug: company + Math.floor(Math.random() * 300),
            platform: {
              connect: {
                id: platform.id,
              },
    ...
    <Section>
      {submitted ? (
        <SubmittedScreen>
          ...
          <SubmittedText>
            A member of the Mercy team will be in touch with you soon.
          </SubmittedText>
          ...
          <CopyRightContainer>
            © {new Date().getFullYear()} Mercy Studio Inc.
          </CopyRightContainer>
        </SubmittedScreen>
      ) : (
        <Formik
