...
class CopyToClipboard extends Component<CopyToClipboardProps, { copied: false }> {
  state = {
    copied: false,
  };
  ...
  render() {
    ...
    return (
      <CopyIconContainer copied={copied} onClick={() => this.copyToClipboardOnClick(copyOnClick)} data-a11y="false">
        {copied ? (
          <CopyIconText>
            <strong>{successText || copyOnClick}</strong> copied to clipboard <CopiedIcon />
          </CopyIconText>
        ) : (
          <>
